...
use crate::{
    ...
    ThreadedWorldLoader, WorldContext, WorldRef, WorldViewer,
};
...
pub struct Simulation<R: Renderer> {
    ...
    terrain_changes: HashSet<WorldTerrainUpdate<WorldContext>>,

    /// World change events populated during terrain updates, consumed every tick
    change_events: Vec<WorldChangeEvent<WorldContext>>,
...
pub struct TerrainUpdatesRes(Vec<WorldTerrainUpdate<WorldContext>>);
...
impl<R: Renderer> Simulation<R> {
    ...
    fn on_world_changes(&mut self, events: &[WorldChangeEvent<WorldContext>]) {
