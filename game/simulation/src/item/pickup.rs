...
pub enum PickupItemError {
    ...
    #[error("Entity does not exist, maybe it has already been picked up")]
    BadEntity,
        ...
        for (holder, pickup, picker_upper_transform) in (&entities, &pickup, &transforms).join() {
            ...
            let mut do_pickup = || {
                ...
                let base_item = match base_items.get(item) {
                    Some(comp) => comp,
                    None if entities.is_alive(item) => return Err(PickupItemError::NotAnItem),
                    None => return Err(PickupItemError::BadEntity),
                };
                ...
                if !picker_upper_transform.position.is_almost(
                    ...
                    trace!("too far away to pick up item";
                        "distance" => distance,
                        "item_pos" => %item_transform.position,
                        "entity_pos" => %picker_upper_transform.position
                    );
                ...
                updates.queue("pick up item", move |world| {
                    ...
                    let event_payload = match result {
                        Ok(_) => {
                            ...
                            debug!("picked up item"; E(holder), "item" => E(item));
            ...
            if let Err(err) = do_pickup() {
                ...
                // abort pickup
                updates.queue("abort picking up item", move |world| {
                    let _ = world.remove_now::<PickupItemComponent>(holder);
                    Ok(())
                });
            }
