...
use crate::activity::status::dyn_slot::DynSlot;
use crate::{dynslot_new, dynslot_update};
...
struct Inner(DynSlot<dyn Display>);
...
mod dyn_slot {
    use common::{PhantomData, SmallVec};
    use std::mem::ManuallyDrop;
    use std::ptr::NonNull;

    pub struct DynSlot<T: ?Sized> {
        vtable: NonNull<()>,
        data: SmallVec<[u8; 64]>,
    ...
    impl<T: ?Sized> Drop for DynSlot<T> {
        fn drop(&mut self) {
