...
extern "system" fn on_debug_message(
    ...
    if gltype == gl::DEBUG_TYPE_ERROR {
        my_error!("GL error"; "severity" => severity, "message" => msg);
    } else {
        ...
        match severity {
            gl::DEBUG_SEVERITY_HIGH => my_warn!("GL message"; o),
            gl::DEBUG_SEVERITY_MEDIUM => my_info!("GL message"; o),
            _ => my_debug!("GL message"; o),
        };
    }
}
