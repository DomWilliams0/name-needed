...
pub struct BlockPosition(pub BlockCoord, pub BlockCoord, pub GlobalSliceIndex);
...
impl From<(BlockCoord, BlockCoord, i32)> for BlockPosition {
    fn from(pos: (BlockCoord, BlockCoord, i32)) -> Self {
        let (x, y, z) = pos;
        assert!(x < CHUNK_SIZE.as_block_coord());
        assert!(y < CHUNK_SIZE.as_block_coord());
        Self(x, y, z.into())
    }
}
...
impl From<(i32, i32, i32)> for BlockPosition {
    fn from(pos: (i32, i32, i32)) -> Self {
        let (x, y, z) = pos;
        assert!(x < CHUNK_SIZE.as_i32(), "x={} is out of range", x);
        assert!(y < CHUNK_SIZE.as_i32(), "y={} is out of range", y);
        Self(x as BlockCoord, y as BlockCoord, SliceIndex::new(z))
    }
}
